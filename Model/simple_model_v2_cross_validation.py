...
class GATModel(nn.Module):
    ...
    def forward(self, data):
        ...
        x_new_s = self.conv2((x_new_t, x_s), edge_index[torch.tensor([1, 0])], size=(x_new_t.size(0), x_s.size(0)),
                             edge_attr=distances)
        x = torch.relu(x_new_t)
...
train_loader = DataLoader(filtered_data_list[:1500], batch_size=2, shuffle=False, follow_batch=['x_s', 'x_t'])
test_loader = DataLoader(filtered_data_list[1500:1938], batch_size=2, shuffle=False, follow_batch=['x_s', 'x_t'])
